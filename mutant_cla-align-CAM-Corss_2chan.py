...
for i in range(len(all_train_data)):
    if all_train_data[i][2] in mutant_names:
        mutant_img[all_train_data[i][2]] = (all_train_data[i][3] - 0.5, all_train_data[i][4] - 0.5, 0)
...
for i in range(len(all_train_data)):
    if all_train_data[i][2] not in mutant_names:
        normal_img[all_train_data[i][5]] = (all_train_data[i][3] - 0.5, all_train_data[i][4] - 0.5, 1)
...
class Mouse_sub_volumes(Dataset):
    ...
    def __getitem__(self, num):
        ...
        current_bv, current_img, label = self.all_data[num]
        
        img = np.float32(np.concatenate((current_bv[np.newaxis,...], current_img[np.newaxis,...]), axis=0))
...
class Flip(object):
    ...
    def __call__(self, sample):
        if random.uniform(0,1) < self.ori_probability:
            ...
            img[0,...] = flipping[random_choise1](img[0,...])
            img[1,...] = flipping[random_choise1](img[1,...])
...
class VGG_net(nn.Module):
    def __init__(self,conv_drop_rate=0.10,linear_drop_rate=0.2):
        ...
        self.conv1 = nn.Conv3d(in_channels=2, out_channels=12, kernel_size=3,stride=1, padding=2,dilation=2)
...
for fold in range(6):
    ...
    print('fold: ', fold, 'test_data_length:', len(test_data), 'train_data_length:', len(train_data))
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            torch.save(net.state_dict(), './model/mut_clas_2019_02_01_e{}_global2_fold{}_2channel.pth'.format(epoch+1,fold))
            print("----------------------------------------------------------")
            print("----------------------------------------------------------")
