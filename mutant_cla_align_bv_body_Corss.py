...
save_name = 'All_unalign_bv_data_64_64_64_down.pickle'
...
for fold_num in range(6):
        ...
        if current_fold == fold_num:
            for i in range(len(data_folds[current_fold])):
                if data_folds[current_fold][i][0] in all_train_data and data_folds[current_fold][i][0] not in used_name_list:
                    # cur_img = np.float32(all_train_data[data_folds[current_fold][i][0]] >= 1.5)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            torch.save(net.state_dict(), './model/mut_clas_2020_03_20_e{}_global3_fold{}_raw_bv.pth'.format(epoch+1,fold_num))
